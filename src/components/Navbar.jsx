...
const LOGO_W = 351;
const LOGO_H = 118;
const LOGO_STYLE = { height: "58px", width: "auto" };

const FOOTER_LINKS = [
  { label: "Canal de Denúncias", href: "https://app.pipefy.com/public/form/dirpZ0Km", ext: true },
  { label: "Política de Privacidade", href: "/privacidade", ext: false },
  { label: "Termos de Uso", href: "/termos", ext: false },
];
  ...
  return (
    <>
      {!open && (
        <div className="fixed right-4 top-5 z-40 md:right-8 md:top-7">
          <motion.button
            ...
            className="group flex h-16 items-center rounded-2xl border border-[#d6d6d6] bg-white/95 px-4 shadow-pill backdrop-blur-xl"
          >
            <div className="relative flex h-[58px] w-48 items-center overflow-hidden">
              <motion.img
                src={logoSmall}
            ...
            <div className="flex flex-col gap-[5px]">
              {[0, 1, 2].map((i) => (
                <span
                  key={i}
                  className="block h-[2px] w-5 rounded-full bg-[#333846] transition-all group-hover:w-6 group-hover:bg-[#1c2846]"
                />
              ))}
            </div>
          </motion.button>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            className="fixed inset-0 z-50 overflow-y-auto bg-[#1c2846] text-white"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            <button
              ...
              className="absolute right-5 top-5 flex h-10 w-10 items-center justify-center rounded-xl border border-white/30 bg-white/5 transition-colors hover:bg-white/12 md:right-8 md:top-7"
            >
              <span className="relative block h-4 w-4">
                <span className="absolute inset-0 m-auto h-[2px] w-full rotate-45 rounded-full bg-white" />
                <span className="absolute inset-0 m-auto h-[2px] w-full -rotate-45 rounded-full bg-white" />
              </span>
            </button>
              ...
              <div className="flex-1 border-b border-white/15 pb-8 md:border-b-0 md:border-r md:border-white/15 md:pr-12">
                ...
                <nav className="mt-6 space-y-4">
                  {NAV_LINKS.map((item) => {
                    ...
                    const inner = (
                      <>
                        <span className="inline-block transition-transform group-hover:translate-x-1 group-hover:drop-shadow-[0_0_16px_rgba(255,255,255,0.55)]">
                          {item.label}
                        </span>
                    ...
                    if (item.ext) {
                      return (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="block text-white/50 transition-all hover:translate-x-1 hover:text-white"
                        >
                          {item.label}
                        </a>
                      );
              ...
              <div className="flex-1">
                ...
                <div className="mt-6 space-y-8">
                  {NAV_HIGHLIGHTS.map((item, i) => {
                    const inner = (
                      ...
                      >
                        <div className="flex w-full items-center justify-between gap-4">
                          ...
                          <span className="flex-none text-lg text-white/50 transition-all group-hover:translate-x-1 group-hover:text-white">
                            →
                          </span>
                        </div>
                    ...
                    return (
                      <button key={i} onClick={() => handleNav(item.id)} className="w-full">
                        {inner}
                      </button>
                    );
                  })}
